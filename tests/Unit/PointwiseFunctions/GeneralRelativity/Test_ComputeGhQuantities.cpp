...
SPECTRE_TEST_CASE("Unit.PointwiseFunctions.GeneralRelativity.GhQuantities",
  ...
  auto shift = make_with_value<tnsr::I<DataVector, 3, Frame::Inertial>>(
      test_vector, 0.0);
  get<0>(shift) = 0.5;
  get<1>(shift) = 0.6;
  get<2>(shift) = 0.7;
  ...
  auto dt_lapse = make_with_value<Scalar<DataVector>>(test_vector, 0.0);
  get(dt_lapse) = 0.04;

  auto dt_shift = make_with_value<tnsr::I<DataVector, 3, Frame::Inertial>>(
      test_vector, 0.0);
  get<0>(dt_shift) = 0.05;
  get<1>(dt_shift) = 0.06;
  get<2>(dt_shift) = 0.07;
  ...
  auto deriv_lapse = make_with_value<tnsr::i<DataVector, 3, Frame::Inertial>>(
      test_vector, 0.0);
  ...
  const auto& gauge_source = GeneralizedHarmonic::gauge_source(
      lapse, dt_lapse, deriv_lapse, shift, dt_shift, deriv_shift,
      spatial_metric, trace_extrinsic_curvature, trace_christoffel_first_kind);
  ...
  const auto box = db::create<
      ...
      db::AddComputeTags<
          ...
          GeneralizedHarmonic::Tags::TraceExtrinsicCurvatureCompute<
              3, Frame::Inertial>,
          GeneralizedHarmonic::Tags::GaugeHCompute<3, Frame::Inertial>,
          GeneralizedHarmonic::Tags::DerivativesOfSpacetimeMetricCompute<
              3, Frame::Inertial>>>(spatial_metric, lapse, shift,
                                    deriv_spatial_metric, deriv_lapse,
                                    deriv_shift, dt_spatial_metric, dt_lapse,
                                    dt_shift);
