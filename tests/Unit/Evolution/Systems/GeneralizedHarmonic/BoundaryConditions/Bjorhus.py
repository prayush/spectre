...
def set_dt_v_psi_constraint_preserving(unit_interface_normal_vector,
                                       three_index_constraint,
                                       char_projected_rhs_dt_v_psi,
                                       char_speeds):
    return char_projected_rhs_dt_v_psi + char_speeds[0] * np.einsum(
        'i,iab->ab', unit_interface_normal_vector, three_index_constraint)


def set_dt_v_zero_constraint_preserving(unit_interface_normal_vector,
                                        four_index_constraint,
                                        char_projected_rhs_dt_v_zero,
                                        char_speeds):
    result = char_projected_rhs_dt_v_zero

    if len(unit_interface_normal_vector) == 2:
    ...
    elif len(unit_interface_normal_vector) == 3:
        ...
        for p in it.permutations(np.arange(len(unit_interface_normal_vector))):
            ...
            result[p[0], :, :] += sgn * char_speeds[
                1] * unit_interface_normal_vector[
                    p[2]] * four_index_constraint[p[1], :, :]
    return result
...
def set_dt_v_minus_constraint_preserving(
    gamma2, inertial_coords, incoming_null_one_form, outgoing_null_one_form,
    ...
    return add_constraint_preserving_terms_to_dt_v_minus(
        incoming_null_one_form, outgoing_null_one_form, incoming_null_vector,
        outgoing_null_vector, projection_ab, projection_Ab, projection_AB,
        constraint_char_zero_plus, constraint_char_zero_minus,
        char_projected_rhs_dt_v_minus,
...
def set_dt_v_minus_constraint_preserving_physical(
...
def error(face_mesh_velocity, normal_covector, normal_vector, pi, phi,
          spacetime_metric, coords, gamma1, gamma2, lapse, gauge_source,
...
def compute_intermediate_vars(face_mesh_velocity, normal_covector, pi, phi,
                              spacetime_metric, inertial_coords, gamma1,
                              gamma2, lapse, gauge_source,
    ...
    extrinsic_curvature = gh.extrinsic_curvature(spacetime_unit_normal_vector,
                                                 pi, phi)
    three_index_constraint = d_spacetime_metric - phi
    ...
    return (unit_interface_normal_vector, three_index_constraint,
            four_index_constraint, inverse_spatial_metric, extrinsic_curvature,
            inverse_spacetime_metric, spacetime_unit_normal_vector,
...
def set_bc_corr_zero_when_char_speed_is_positive(dt_v_corr, char_speeds):
    dt_v_corr[char_speeds > 0.0] = 0
    return dt_v_corr


def dt_corrs_ConstraintPreserving(face_mesh_velocity, normal_covector,
                                  normal_vector, spacetime_metric, pi, phi,
                                  coords, gamma1, gamma2, lapse, gauge_source,
                                  spacetime_deriv_gauge_source,
                                  dt_spacetime_metric, dt_pi, dt_phi,
                                  d_spacetime_metric, d_pi, d_phi):
    (unit_interface_normal_vector, three_index_constraint,
     four_index_constraint, inverse_spatial_metric, extrinsic_curvature,
     inverse_spacetime_metric, spacetime_unit_normal_vector,
     incoming_null_one_form, outgoing_null_one_form, incoming_null_vector,
     outgoing_null_vector, projection_ab, projection_Ab, projection_AB,
...
def dt_corrs_ConstraintPreservingPhysical(
    ...
    dt_v_zero = set_dt_v_zero_constraint_preserving(
        ...
        char_projected_rhs_dt_v_zero, char_speeds)
    dt_v_plus = dt_v_psi * 0
    dt_v_minus = set_dt_v_minus_constraint_preserving_physical(
...
def dt_spacetime_metric(face_mesh_velocity, normal_covector, normal_vector,
                        ...
                        gamma2, lapse, gauge_source,
                        spacetime_deriv_gauge_source, dt_spacetime_metric,
                        dt_pi, dt_phi, d_spacetime_metric, d_pi, d_phi):
    (dt_v_psi, dt_v_zero,
     dt_v_plus, dt_v_minus) = dt_corrs_ConstraintPreserving(
         face_mesh_velocity, normal_covector, normal_vector, spacetime_metric,
         pi, phi, inertial_coords, gamma1, gamma2, lapse, gauge_source,
         spacetime_deriv_gauge_source, dt_spacetime_metric, dt_pi, dt_phi,
         d_spacetime_metric, d_pi, d_phi)
    return -dt_spacetime_metric + dt_v_psi


def dt_pi_ConstraintPreserving(face_mesh_velocity, normal_covector,
                               normal_vector, spacetime_metric, pi, phi,
                               inertial_coords, gamma1, gamma2, lapse,
                               gauge_source, spacetime_deriv_gauge_source,
                               dt_spacetime_metric, dt_pi, dt_phi,
                               d_spacetime_metric, d_pi, d_phi):
    (dt_v_psi, dt_v_zero,
     dt_v_plus, dt_v_minus) = dt_corrs_ConstraintPreserving(
         ...
         spacetime_deriv_gauge_source, dt_spacetime_metric, dt_pi, dt_phi,
         d_spacetime_metric, d_pi, d_phi)
    return -dt_pi + ght.evol_field_pi(gamma2, dt_v_psi, dt_v_zero, dt_v_plus,
                                      dt_v_minus, normal_covector)


def dt_pi_ConstraintPreservingPhysical(
    face_mesh_velocity, normal_covector, normal_vector, spacetime_metric, pi,
    phi, inertial_coords, gamma1, gamma2, lapse, gauge_source,
    spacetime_deriv_gauge_source, dt_spacetime_metric, dt_pi, dt_phi,
    d_spacetime_metric, d_pi, d_phi):
...
def dt_phi_ConstraintPreserving(face_mesh_velocity, normal_covector,
                                normal_vector, spacetime_metric, pi, phi,
                                inertial_coords, gamma1, gamma2, lapse,
                                gauge_source, spacetime_deriv_gauge_source,
                                dt_spacetime_metric, dt_pi, dt_phi,
    ...
    (dt_v_psi, dt_v_zero,
     dt_v_plus, dt_v_minus) = dt_corrs_ConstraintPreserving(
         ...
         spacetime_deriv_gauge_source, dt_spacetime_metric, dt_pi, dt_phi,
         d_spacetime_metric, d_pi, d_phi)
    return -dt_phi + ght.evol_field_phi(gamma2, dt_v_psi, dt_v_zero, dt_v_plus,
                                        dt_v_minus, normal_covector)
