...
void test_upwind_flux_consistency_random() noexcept {
  ...
  const auto unit_normal_one_form_ext = StrahlkorperGr::unit_normal_one_form(
      minus_unit_normal_one_form_int, one_over_one_form_magnitude_ext);
  ...
  tnsr::i<DataVector, spatial_dim, Frame::Inertial>
      minus_unit_normal_one_form_ext = unit_normal_one_form_ext;
  for (size_t i = 0; i < spatial_dim; ++i) {
    minus_unit_normal_one_form_ext.get(i) *= -1.0;
  }
  ...
  auto psi_normal_dot_numerical_flux = make_with_value<
      ...
      one, std::numeric_limits<double>::signaling_NaN());
...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.GeneralizedHarmonic.UpwindFlux",
                  "[Unit][Evolution]") {
  {
    INFO("test consistency of the upwind flux")
    test_upwind_flux_consistency_random();
  }
}
