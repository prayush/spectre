...
class ObserveNorms<ObservationValueTag, tmpl::list<Tensors...>, EventRegistrars>
 ...
 public:
  ...
  static constexpr OptionString help =
      ...
      " * ObservationValueTag\n"
      " * NumberOfPoints = total number of points in the domain\n"
      " * Norm(*) = RMS errors in Tensors (see online help details)\n"
      "\n"
      "Warning: Currently, only one reduction observation event can be\n"
  ...
  void operator()(const typename ObservationValueTag::type& observation_value,
    ...
    tuples::TaggedTuple<LocalSquaredValue<Tensors>...> local_squared_values;
    const auto record_errors = [&local_squared_values](
        const auto tensor_tag_v, const auto& tensor) noexcept {
      using tensor_tag = tmpl::type_from<decltype(tensor_tag_v)>;
      double local_square_value = 0.0;
      for (size_t i = 0; i < tensor.size(); ++i) {
        local_square_value += alg::accumulate(square(tensor[i]), 0.0);
      }
      get<LocalSquaredValue<tensor_tag>>(local_squared_values) =
          local_square_value;
      return 0;
    };
    ...
    Parallel::simple_action<observers::Actions::ContributeReductionData>(
        ...
        ReductionData{static_cast<double>(observation_value), num_points,
                      std::move(get<LocalSquaredValue<Tensors>>(
                          local_squared_values))...});
  }
};
