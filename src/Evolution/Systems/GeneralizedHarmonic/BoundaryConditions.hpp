  ...
  struct apply_impl {
    static std::tuple<db::DataBox<DbTags>&&> function_impl(
      ...
      for (auto& external_direction_and_normals : unit_normal_one_forms) {
        ...
        if (debugPKoff) {
          const auto& inertial_x = inertial_coords.get(0);
          const auto& inertial_y = inertial_coords.get(1);
          const auto& inertial_z = inertial_coords.get(2);
          std::cout << "Coords:" << std::endl;
          for (size_t i = 0; i < inertial_x.size(); ++i) {
            std::cout << "(" << inertial_x[i] << ", " << inertial_y[i] << ", "
                      << inertial_z[i] << ")" << std::endl
                      << std::flush;
          }

          const auto& local_psi =
              get<gr::Tags::SpacetimeMetric<VolumeDim, Frame::Inertial>>(vars);
          const auto& local_phi =
              get<Tags::Phi<VolumeDim, Frame::Inertial>>(vars);
          const auto& local_pi =
              get<Tags::Pi<VolumeDim, Frame::Inertial>>(vars);

          const auto& local_dtpsi = get<::Tags::dt<
              gr::Tags::SpacetimeMetric<VolumeDim, Frame::Inertial>>>(dt_vars);
          // const auto& local_dtphi =
          // get<::Tags::dt<Tags::Phi<VolumeDim, Frame::Inertial>>>(dt_vars);
          const auto& local_dtpi =
              get<::Tags::dt<Tags::Pi<VolumeDim, Frame::Inertial>>>(dt_vars);

          const auto& local_lapse =
              get(get<::Tags::Interface<
                      ::Tags::BoundaryDirectionsInterior<VolumeDim>,
                      gr::Tags::Lapse<DataVector>>>(box)
                      .at(direction));
          const auto& local_shift = get<::Tags::Interface<
              ::Tags::BoundaryDirectionsInterior<VolumeDim>,
              gr::Tags::Shift<VolumeDim, Frame::Inertial, DataVector>>>(box)
                                        .at(direction);

          const auto& local_h = get<::Tags::Interface<
              ::Tags::BoundaryDirectionsInterior<VolumeDim>,
              Tags::GaugeH<VolumeDim, Frame::Inertial>>>(box)
                                    .at(direction);
          const auto& local_d3h = get<::Tags::Interface<
              ::Tags::BoundaryDirectionsInterior<VolumeDim>,
              Tags::SpacetimeDerivGaugeH<VolumeDim, Frame::Inertial>>>(box)
                                      .at(direction);

          const auto& local_gamma0 =
              get(get<::Tags::Interface<
                      ::Tags::BoundaryDirectionsInterior<VolumeDim>,
                      Tags::ConstraintGamma0>>(box)
                      .at(direction));
          const auto& local_gamma1 =
              get(get<::Tags::Interface<
                      ::Tags::BoundaryDirectionsInterior<VolumeDim>,
                      Tags::ConstraintGamma1>>(box)
                      .at(direction));
          const auto& local_gamma2 =
              get(get<::Tags::Interface<
                      ::Tags::BoundaryDirectionsInterior<VolumeDim>,
                      Tags::ConstraintGamma2>>(box)
                      .at(direction));

          const auto& local_3idx_constraint = get<::Tags::Interface<
              ::Tags::BoundaryDirectionsInterior<VolumeDim>,
              Tags::ThreeIndexConstraint<VolumeDim, Frame::Inertial>>>(box)
                                                  .at(direction);

          // Equation for dt_spacetime_metric
          db::item_type<
              ::Tags::dt<gr::Tags::SpacetimeMetric<VolumeDim, Frame::Inertial>>>
              local_dt_spacetime_metric{inertial_x.size()};

          // In here we check how one gets roundoff fluctuations in computation
          // of dt<spacetime metric>
          if (false) {
            for (size_t mu = 0; mu < VolumeDim + 1; ++mu) {
              for (size_t nu = mu; nu < VolumeDim + 1; ++nu) {
                local_dt_spacetime_metric.get(mu, nu) =
                    -local_lapse * local_pi.get(mu, nu);
                //
                if (mu == 0 and nu == 0) {
                  std::cout << "Dt<PSI>[0,0]: "
                            << local_dt_spacetime_metric.get(0, 0)[0] << "\t\t"
                            << "1 + Gamma1: " << (1 + local_gamma1)[0]
                            << std::endl
                            << std::flush;
                }
                //
                for (size_t m = 0; m < VolumeDim; ++m) {
                  // local_dt_spacetime_metric.get(mu, nu) +=
                  //  (1 + local_gamma1) * local_shift.get(m) *
                  //  local_3idx_constraint.get(m, mu, nu);
                  local_dt_spacetime_metric.get(mu, nu) +=
                      local_shift.get(m) * local_phi.get(m, mu, nu);
                  //
                  if (mu == 0 and nu == 0) {
                    std::cout
                        << "Dt<PSI>[0,0]: "
                        << local_dt_spacetime_metric.get(mu, nu)[0]
                        << "\t\t shift, phi: " << local_shift.get(m)[0] << ", "
                        << local_phi.get(m, mu, nu)[0] << " :: "
                        << (local_shift.get(m) * local_phi.get(m, mu, nu))[0]
                        << std::endl
                        << std::flush;
                  }
                }
          ...
          for (size_t i = 0; i < inertial_x.size(); ++i) {
            std::cout << "--- OUTPUT for Gamma(0, 1, 2)[" << inertial_x[i]
                      << ", " << inertial_y[i] << ", " << inertial_z[i]
                      << "]: " << local_gamma0[i] << ", " << local_gamma1[i]
                      << ", " << local_gamma2[i] << ";" << std::endl
                      << std::endl
                      << std::flush;
            //
            std::cout << "--- OUTPUT for Lapse[" << inertial_x[i] << ", "
                      << inertial_y[i] << ", " << inertial_z[i]
                      << "]: " << local_lapse[i] << ";" << std::endl
                      << std::endl
                      << std::flush;
            //
            print_rank1_tensor_at_point("Shift", local_shift, inertial_x,
                                        inertial_y, inertial_z, i);
            //
            print_rank2_tensor_at_point("Psi", local_psi, inertial_x,
                                        inertial_y, inertial_z, i);
            //
            print_rank1_tensor_at_point("GaugeH", local_h, inertial_x,
                                        inertial_y, inertial_z, i);
            //
            print_rank2_tensor_at_point("D4GaugeH", local_d3h, inertial_x,
                                        inertial_y, inertial_z, i);
            //
            print_rank2_tensor_at_point("Dt<Psi>", local_dtpsi, inertial_x,
                                        inertial_y, inertial_z, i);
            //
            print_rank2_tensor_at_point("RECALCULATED Dt<Psi>",
                                        local_dt_spacetime_metric, inertial_x,
                                        inertial_y, inertial_z, i);
            //
            print_rank2_tensor_at_point("Pi", local_pi, inertial_x, inertial_y,
                                        inertial_z, i);
            //
            print_rank2_tensor_at_point("Dt<Pi>", local_dtpi, inertial_x,
                                        inertial_y, inertial_z, i);
            //
            print_rank3_tensor_at_point("Phi", local_phi, inertial_x,
                                        inertial_y, inertial_z, i);
            //
            std::cout << "--- OUTPUT for D3<Psi>[" << inertial_x[i] << ", "
                      << inertial_y[i] << ", " << inertial_z[i] << "] ---"
                      << std::endl
                      << std::flush;
            for (size_t j = 0; j < VolumeDim; ++j) {
              for (size_t a = 0; a <= VolumeDim; ++a) {
                for (size_t b = 0; b <= VolumeDim; ++b) {
                  std::cout << "  "
                            << local_phi.get(j, a, b)[i] +
                                   local_3idx_constraint.get(j, a, b)[i]
                            << "  " << std::flush;
                }
