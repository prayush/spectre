...
void set_dt_v_zero_constraint_preserving(
  ...
  for (size_t a = 0; a <= VolumeDim; ++a) {
    for (size_t b = a; b <= VolumeDim; ++b) {
      ...
      if (UNLIKELY(VolumeDim == 2)) {
        // Lets say this term is T2_{kab} := - n_l N^l n^j C_{jkab}.
        // In 2+1 spacetime, we store the four index constraint to
        // be C4_{1ab} = C_{12ab}, C_{2ab} = C_{21ab}. Therefore,
        // T_{kab} = -n_l N^l (n^1 C_{1kab} + n^2 C_{2kab}), i.e.
        // T_{1ab} = -n_l N^l n^2 C4_{2ab}, T_{2ab} = -n_l N^l n^1 C4_{1ab}.
        bc_dt_v_zero->get(0, a, b) +=
            char_speeds.at(1) * (unit_interface_normal_vector.get(1) *
                                 four_index_constraint.get(1, a, b));
        bc_dt_v_zero->get(1, a, b) +=
            char_speeds.at(1) * (unit_interface_normal_vector.get(0) *
                                 four_index_constraint.get(0, a, b));
      } else if (LIKELY(VolumeDim == 3)) {
        // Lets say this term is T2_{kab} := - n_l N^l n^j C_{jkab}.
