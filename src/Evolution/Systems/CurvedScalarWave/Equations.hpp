 ...
 *
 * \f{align*}
 * F(\psi) &= -(1 + \gamma_1) N^k n_k \psi \\
 * F(\Pi) &= - N^k n_k \Pi + N g^{ki}n_k \Phi_{i}
 *           - \gamma_1 \gamma_2 N^k n_k \Psi  \\
 * F(\Phi_{i}) &= - N^k n_k \Phi_{i} + N n_i \Pi - \gamma_2 N n_i \psi
 * \f}
 *
 ...
 * derivative of \f$\psi\f$, \f$N\f$ is the lapse, \f$ N^k \f$ is the shift,
 * \f$ g^{ki} \f$ is the inverse spatial metric, and \f$ \gamma_1, \gamma_2\f$
 * are constraint damping parameters. Note that the last term in \f$F(\Pi)\f$
 * will be identically zero, as it is necessary to set \f$\gamma_1\gamma_2=0\f$
 * in order to make this first-order formulation of the curved scalar wave
...
struct ComputeNormalDotFluxes {
 public:
  using argument_tags = tmpl::list<
      Pi, Phi<Dim>, Psi, Tags::ConstraintGamma1, Tags::ConstraintGamma2,
 ...
 * where
 *
 *  - \f$F^*\f$ is the numerical upwind flux dotted with the interface normal;
 ...
 * Further, the sign of characteristic speeds indicates the direction of
 * propagation of the corresponding characteristic field with respect to the
 * interface normal that the field has been
 * computed along (with negative speeds indicating incoming characteristics,
 * and vice-versa). Therefore, \f$\Lambda^+\f$ contains characterstic speeds
 * for variables that are outgoing from the element at the interface in
 * question, and vice-versa for \f$\Lambda^-\f$. An ambiguity naturally arises
 * as to which set of
 * evolved variables (''interior'' or ''exterior'') to use when computing these
 * speeds. Following the treatment of generalized harmonic evolution system's
 * characteristics in its upwind flux (\ref GeneralizedHarmonic::UpwindFlux ),
 * here also we compute both and use their average, i.e.
 ...
 *  -# Computes characteristic variables and speeds using evolved variables from
 ...
 *
 * \f[
 * F^* = S\left(w_{\rm ext} \hat{U}^{\rm ext}
 ...
 * \f${F^*}^{\rm int}(\hat{U}^{\rm int}, \hat{U}^{\rm ext})
 * = - {F^*}^{\rm ext}(\hat{U}^{\rm ext}, \hat{U}^{\rm int})\f$,
