 ...
 * system. This is intended as a map for maintainers of the codebase. If you are
 * interested primarily in running SpECTRE CCE to obtain waveform data, please
 * see the executable documentation.
 *
 * First, worldtube data from a completed or running GH evolution must be
 * translated to Bondi spin-weighted scalars at the extraction sphere. Relevant
 * utilities for this conversion are `Cce::WorldtubeDataManager`,
 * `Cce::ReducedWorldtubeDataManager`, `Cce::create_bondi_boundary_data`.
 ...
 * right-hand side of the equation must be computed, then the radial
 * differential equation integrated. The equations have a hierarchical
 * structure, so the result for \f$\beta\f$ feeds into the right-hand side for
 * \f$Q\f$, and both feed into \f$U\f$, and so on. Relevant utilities for
 * computing the inputs to the hypersurface equations are
 ...
 * Note that some of the hypersurface equations are not of simple form, and
 * therefore require more careful techniques than a direct indefinite radial
 * integration.
 *
 ...
 * evaluated is the simulation time, not an aymptotically inertial time. So,
 ...
 * reliable interpolation at a consistent cut of \f$I^+\f$ at constant inertial
 * time. Utilities for calculating and evolving the asymptotic inertial time are
 * `Cce::InitializeScriPlusValue` and `Cce::CalculateScriPlusValue` using
 * arguments involving `Cce::Tags::InertialRetardedTime`. A utility for managing
 * the interpolation is `Cce::ScriPlusInterpolationManager`, and relevant parts
 * of the parallel infrastructure for manipulating the data into the
 * interpolator and writing the results to disk are
 * `Cce::Actions::EnqueueInterpolationScriData` and
