...
def generate_xdmf(file_prefix, output_filename, start_time, stop_time, stride):
    ...
    for id_and_value in temporal_ids_and_values:
        ...
        for h5file in h5files:
            ...
            number_of_cells = sum(
                (extents_x - 1) * (extents_y - 1) * (extents_z - 1))
...
def generate_xdmf_2d(file_prefix, output_filename,
                     start_time, stop_time, stride):
    """
    Generate one XDMF file that ParaView and VisIt can use to load the data
    out of the HDF5 files.
    """
    ...
    for id_and_value in temporal_ids_and_values:
        ...
        for h5file in h5files:
            ...
            extents = h5temporal.get("total_extents")
            extents_x = np.array([extents[i] for i in
                                  range(extents.size) if i % 2 == 0])
            extents_y = np.array([extents[i] for i in
                                  range(extents.size) if i % 2 == 1])

            total_extents_x = sum(extents_x)
            total_extents_y = sum(extents_y)
            numpoints = sum(extents_x * extents_y)
            number_of_cells = sum((extents_x - 1) * (extents_y - 1))
            data_item = "        <DataItem Dimensions=\" %d\" " \
                "NumberType=\"Double\" Precision=\"8\" Format=\"HDF5\">\n" % (
                    numpoints)
            data_item_vec = "        <DataItem Dimensions=\" %d 2\" " \
                "ItemType = \"Function\" Function = \"JOIN($0,$1)\">\n" % (
                    numpoints)
            Grid_path = "          %s:/element_data.vol/%s" % (
                h5file[1], id_and_value[0])
            xdmf_output += "    <Grid Name=\"%s\" GridType=\"Uniform\">\n" % (
                h5file[1])
            # Write topology information
            xdmf_output += "      <Topology TopologyType=\"Quadrilateral\" " \
                "NumberOfElements=\"%d\">\n" % (number_of_cells)
            xdmf_output += "        <DataItem Dimensions=\"%d 4\"  " \
                "NumberType=\"Int\" Format=\"HDF5\">\n" % (number_of_cells)
            xdmf_output += Grid_path + \
                "/connectivity\n        </DataItem>\n      </Topology>\n"
            # Write geometry/coordinates
            ...
            for component in components:
                if component.endswith("_x"):
                    ...
                    # The component is a y component of a vector
                    # it will be processed with the x component
                    continue
                else:
                    # If the component is not part of a vector,
...
def parse_args():
    ...
    parser.add_argument(
        "--two-dimensional",
        action='store_true',
        help="Operate on volume data in 2 spatial dimensions.")
    return parser.parse_args()


if __name__ == "__main__":
    input_args = parse_args()
    if not input_args.two_dimensional:
        generate_xdmf(input_args.file_prefix, input_args.output,
                      input_args.start_time, input_args.stop_time,
                      input_args.stride)
    else:
        generate_xdmf_2d(input_args.file_prefix, input_args.output,
                         input_args.start_time, input_args.stop_time,
                         input_args.stride)
